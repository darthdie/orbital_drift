...
const layer = createLayer(id, baseLayer => {
    ...
    const eruptionPenalityDisplay = computed(() =>
        Decimal.add(eruptionPressureDivisor, 1).div(maficEffect.value)
    );
    ...
    const volcanicsMax = computed(() =>
        Decimal.fromNumber(100).times(Decimal.times(volcanicsCapIncreases.value, 2).clampMin(1))
    );

    const lava = createResource<DecimalSource>(0, "Lava");
    const lavaTotal = trackTotal(lava);
    const lavaMax = computed(() =>
  ...
  */
    ...
    if (testPriority === PRIORITY_VOLCANICS) {
        ...
        testConversionRate = Decimal.times(
    ...
    baseLayer.on("preUpdate", diff => {
        ...
        if (
            ...
            Decimal.lt(volcanics.value, volcanicsMax.value)
        ) {
            ...
            volcanics.value = Decimal.add(volcanics.value, producedVolcanics).clampMax(
                volcanicsMax.value
            );
        ...
        if (Decimal.gt(testResourceX.value, 0)) {
            ...
            const conversionAmount = Decimal.min(
                testResourceX.value,
                Decimal.times(conversionRate, testDiff)
            );
            const producedVolcanics = Decimal.times(conversionAmount, testConversionRate);

            testResourceY.value = Decimal.add(testResourceY.value, producedVolcanics).clampMax(
                1000
            );
            testResourceX.value = Decimal.sub(testResourceX.value, conversionAmount);

            if (testLoss) {
                testResourceX.value = Decimal.sub(
                    testResourceX.value,
                    Decimal.times(testResourceX.value, testLossRate).times(testDiff)
                );
            }
        }
    });
    ...
    const pressureChanceBuyableEffect = createSequentialModifier(() => [
        createAdditiveModifier(() => ({
            ...
            addend: () =>
                Decimal.div(pressureBuyables.pressureChance.amount.value, 4)
                    .add(luckyCastIron.value)
                    .add(sevensEffect.value)
        }))
    ]);
    ...
    const pressureMultBuyableEffect = computed(() => {
        ...
        return Decimal.times(0.1, pressureBuyables.pressureMult.amount.value)
            .add(1)
            .times(diamondsEffect.value);
    });
    ...
    const pressureBuyables = {
        pressureChance: createRepeatable(() => ({
            requirements: createCostRequirement(
                (): CostRequirementOptions => ({
                    ...
                    cost: Formula.variable(pressureBuyables.pressureChance.amount)
                        .pow_base(1.2)
                        .times(3)
                })
            ),
        ...
        pressureMult: createRepeatable(() => ({
            requirements: createCostRequirement(
                (): CostRequirementOptions => ({
                    resource: noPersist(lava),
                    cost: Formula.variable(pressureBuyables.pressureMult.amount)
                        .pow_base(1.35)
                        .times(5)
                })
            ),
        ...
        pressureInterval: createRepeatable(() => ({
            requirements: createCostRequirement(
                (): CostRequirementOptions => ({
                    resource: noPersist(lava),
                    cost: Formula.variable(pressureBuyables.pressureInterval.amount)
                        .pow_base(1.45)
                        .times(8)
                })
            ),
    ...
    const floorIsLavaEffect = computed(() => {
        if (pressureUpgrades.floorIsLava.bought.value) {
            return Formula.variable(lava.value)
                .pow(0.5)
                .sqrt()
                .clampMin(1)
                .step(2, f => f.sqrt())
    ...
    const hotPotEffect = computed(() => {
        if (pressureUpgrades.hotPot.bought.value) {
            return Formula.variable(new Decimal(pressure.value).e)
                .times(0.1)
                .add(1)
                .step(1.5, f => f.div(5))
                .clampMin(1)
    ...
    const diamondsEffect = computed(() =>
        pressureUpgrades.diamonds.bought.value ? Decimal.fromNumber(2) : Decimal.dOne
    );
    const sevensEffect = computed(() =>
        pressureUpgrades.sevens.bought.value ? Decimal.fromNumber(7.77) : Decimal.dZero
    );
    ...
    const volcanicsBuyables = {
        buckForYourBang: createRepeatable(() => ({
            requirements: createCostRequirement(
                (): CostRequirementOptions => ({
                    ...
                    cost: Formula.variable(volcanicsBuyables.buckForYourBang.amount)
        ...
        shorterFuse: createRepeatable(() => ({
            requirements: createCostRequirement(
                (): CostRequirementOptions => ({
                    resource: noPersist(volcanics),
                    cost: Formula.variable(volcanicsBuyables.shorterFuse.amount)
    ...
    const tephraPressureGainEffect = computed(() =>
        tephraBuyableEffect(tephraBuyables.pressureGain)
    );
    const tephraLavaGainEffect = computed(() => tephraBuyableEffect(tephraBuyables.lavaGain));
    const tephraVolcanicsGainEffect = computed(() =>
        tephraBuyableEffect(tephraBuyables.volcanicsGain)
    );
    const tephraPressureChanceEffect = computed(() =>
        tephraBuyableEffect(tephraBuyables.pressureChance, 0.05)
    );
    const tephraPressureIntervalEffect = computed(() => {
        if (Decimal.gt(tephraBuyables.pressureInterval.amount.value, 0)) {
            return Decimal.sub(
                1,
                Decimal.times(tephraBuyables.pressureInterval.amount.value, 0.01)
            ).clampMin(0.01);
        }
    ...
    const autoLava = createLazyProxy(() => {
        setupPassiveGeneration(layer, lavaConversion, () =>
            pressureCapped.value || Decimal.eq(volcanics.value, volcanicsMax.value)
                ? 0
                : lavaGeneraterEffect.value
        );
            ...
            return (
                <>
                    <span>
                        ...
                        <br />
                        {gainDisplay(lavaConversion, lavaMax.value)}
                        <br />
    ...
    const lavaConversionPriorityEffectsDisplay = computed<Renderable>(() => {
        ...
        if (lavaConversionPriority.value === PRIORITY_VOLCANICS) {
            effectDisplay.push(
                <h6>
                    ...
                    slower, but you gain x{format(lavaConversionPriorityVolcanicsGainEffect.value)}{" "}
        ...
        if (Decimal.gt(lavaConversionLossRateSeconds.value, 0)) {
            effectDisplay.push(
                <h6>
                    You will lose {format(lavaConversionLossRateSeconds.value)}% of lava per second.
                </h6>
            );
        }
        ...
        return (
            <>
                <h6>
                    1 {lava.displayName} will convert into {format(lavaConversionAmount.value)}{" "}
                    {volcanics.displayName} every {format(lavaConversionRateSeconds.value)} seconds.
                </h6>
                <h6>
    ...
    const createResourceDisplay = (
        ...
        return computed(() => (
            <div class="cappable-resource-container">
            ...
            tab: createTab(() => ({
                display: () => (
                    <>
                        ...
                        <h3>
                            {format(lava.value)} {lava.displayName}
                        </h3>
                        <Spacer />
                        ...
                        <div class="flex" style="gap: 16px; justify-content: center;">
                            ...
                            <Slider
                                ...
                                onUpdate:modelValue={value =>
                                    (lavaConversionPriority.value = value)
                                }
        ...
        tephra: () => ({
            ...
            tab: createTab(() => ({
                display: () => (
                    <>
                        <h2>
                            {displayResource(tephra)} {tephra.displayName}
                        </h2>
                        <Spacer />
